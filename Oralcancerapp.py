...
@author: USER
# pyinstaller -p utils -p model_resnet -p visualize -w main_debug.py
"""
...
import utils
import visualize
import model_resnet_cascade
import model_resnet
import model_densenet
import model_densenet_cascade
import model_xception
import model_xception_cascade
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        #self.texth5Browser = QtWidgets.QTextBrowser(self.centralwidget)
        #self.texth5Browser.setGeometry(QtCore.QRect(700, 40, 191, 281))
        #self.texth5Browser.setObjectName("texth5Browser")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.labelresult.setText(self._translate("MainWindow", "result image"))

class PyQtMainEntry(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.list_widget, self.list_widget_h5, self.list_widget_label = self.setupUi(self)
        self.setWindowTitle("Oral cancer detection UI")
        self.filenames = []
        self.model_name = ''
        self.main_model = 'mask'
        self.index_i = -1
    ...
    def btnTest_Pressed(self):
        ...
        if not os.path.exists('logs'):
            os.mkdir('logs')
            if len(glob.glob('logs/*.h5')) == 0:
                QMessageBox.information(self,"Hint","Weight files not exist. Please put the h5 file in document 'logs' first.")
                return
        else:
            if len(glob.glob('logs/*.h5')) == 0:
                QMessageBox.information(self,"Hint","Weight files not exist. Please put the h5 file in document 'logs' first.")
        ...
        if len(self.model_name) == 0:
            QMessageBox.information(self,"Hint","Please select one h5 file first.")
            return
        
        self.textBrowser.append("Image inferencing... (" + self.model_name + ")")
        
        # self.list_widget_h5.itemDoubleClicked.connect(self.onClicked)
    ...
    def btnInput_Clicked(self):
        ...
        # print('self.file_root:', self.file_root)
        
        filename = glob.glob(self.file_root + '*.png')
        
        # print('all filename:', filename)
        self.textBrowser.setPlainText("Open image success")
        self.add_image_names(filename)
        
        self.list_widget_h5.clear()
        
        self.filenames_h5 = []
        
        for i in glob.glob('logs/*.h5'):
            item = QListWidgetItem(i)
            self.list_widget_h5.addItem(item)
            self.filenames_h5.append(i)
        
        self.list_widget_h5.itemDoubleClicked.connect(self.onClicked_h5)
    ...
    def onClicked(self, item):
        ...
        if not os.path.exists('json_test/lesion_oralCa_20200319~20200924_{}_merge.json'.format(filename.split('/')[-1].split('\\')[-1].split('_')[0])):
            ...
            for i in range(len(predicted_information['regions'][0])):
                # print(predicted_information['regions'][0]['region_{}'.format(i+1)])
    ...
    def onClicked_h5(self, item):
        i = item.text()
        self.model_name_total = i
        self.model_name = i.split('_')[-4]
        self.main_model = i.split('\\')[-1].split('_')[0]
        ...
        print('self.main_model:', self.main_model)
        
        if self.main_model == 'cascade' and (self.model_name == 'resnet50' or self.model_name == 'resnet101' or self.model_name == 'resnet152'):
            config.MINI_MASK_SHAPE = (28, 28)
            self.model = model_resnet_cascade.Cascade_MaskRCNN(mode="inference", model_dir='logs', config=config)
        elif self.main_model == 'mask' and (self.model_name == 'resnet50' or self.model_name == 'resnet101' or self.model_name == 'resnet152'):
            config.MINI_MASK_SHAPE = (28, 28)
            self.model = model_resnet.MaskRCNN(mode="inference", model_dir='logs', config=config)
        elif self.main_model == 'cascade' and self.model_name == 'densenet':
            config.MINI_MASK_SHAPE = (56, 56)
            self.model = model_densenet_cascade.Cascade_MaskRCNN(mode="inference", model_dir='logs', config=config)
        elif self.main_model == 'mask' and self.model_name == 'densenet':
            config.MINI_MASK_SHAPE = (56, 56)
            self.model = model_densenet.MaskRCNN(mode="inference", model_dir='logs', config=config)
        elif self.main_model == 'cascade' and self.model_name == 'xception':
            config.MINI_MASK_SHAPE = (56, 56)
            self.model = model_xception_cascade.Cascade_MaskRCNN(mode="inference", model_dir='logs', config=config)
        elif self.main_model == 'mask' and self.model_name == 'xception':
            config.MINI_MASK_SHAPE = (56, 56)
            self.model = model_xception.MaskRCNN(mode="inference", model_dir='logs', config=config)
    ...
    def btnTest_Clicked(self):
        ...
        if not hasattr(self, "captured"):
            ...
            QMessageBox.information(self,"Hint","No image found. Please put the images in any document first.")
            return
        ...
        #model.load_weights("./logs/mask_rcnn_resnet152_microcontroller_detection_0200.h5", by_name=True)
        
        if len(self.model_name) == 0:
            QMessageBox.information(self, 'Hint', 'Please select one h5 file on the top right textbox.')
            return
        
        self.model.load_weights(self.model_name_total, by_name=True)
        ...
        plt.savefig('temp/' + self.image_info["name"].split('\\')[-1], bbox_inches='tight')
        plt.close()
    ...
    def btnTest_total_Clicked(self):
        ...
        for i in range(num):
            ...
            plt.imshow(image_display)
            plt.savefig('temp/' + self.filenames[i].split('/')[-1].split('\\')[-1], bbox_inches='tight')
            plt.close()
    ...
    def label_update_append(self, action, class_index):
        ...
        while True:
            ...
            print('k:', k)
            print("len(data['xpoints']):", len(data['xpoints']))
    ...
    def image_update(self):
        ...
        pic = cv.imread(self.file_root + self.image_info['name'].split('\\')[-1])
    ...
    def display_image(self):
        #global fname_i
        #print('item2.text():', item2.text())
        #QMessageBox.information(self, "Info", item.text())
        
        pic = cv.cvtColor(cv.imread(self.file_root + self.image_info["name"].split('\\')[-1]), cv.COLOR_BGR2RGB)
        pic = cv.resize(pic, (512, 384), interpolation=cv.INTER_AREA)
    ...
    def onClicked2(self, item2):
        ...
        #print("self.index_i:", self.index_i)
        #QMessageBox.information(self, "Info", item.text())
    ...
    def btnSave_Clicked(self):
        ...
        #print("self.image_info['labels']:", len(self.image_info['labels']))
        
        for i in range(len(self.image_info['labels'])):
            segmentation = []
            all_xpoints = []
            all_ypoints = []
            #print(self.image_info['xpoints'])
            #print(self.image_info['ypoints'])
